...
class InvestmentHelpSpec extends Spec {
  ...
  private val laterMs = nowMs + Settings.msPerDay
  private val msPerYear = Settings.msPerDay * Settings.daysPerYear
  ...
  object `Investment Help (implicit class)` {
    ...
    object `investment interval` {
      ...
      def `should be calculated in years`(): Unit = {
        testInvestment.years should equal((1D / Settings.daysPerYear) +- .001)
      }
