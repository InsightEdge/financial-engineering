...
import org.insightedge.examples.financialengineering.model.{TickerSymbol, TickerSymbolProperties}
...
object TickerSymbols extends SpaceUsage {
  ...
  private def isLocked(tickerSymbol: TickerSymbol, propertyName: String): Boolean = {
    propertyName match {
      case `ingestionPropName` =>
        tickerSymbol.ingestionThreadCount >= CoreSettings.ingestionThreadsPerSymbol
      case `feedPropName` =>
       ...
       */
      case `calcIndividualPropName` =>
        tickerSymbol.calcIndividualReturnThreadCount >= CoreSettings.calcIndividualThreadsPerSymbol
      case `calcMarketPropName` =>
        tickerSymbol.calcMarketReturnThreadCount >= CoreSettings.calcMarketReturnThreadsPerSymbol
      case _ =>
